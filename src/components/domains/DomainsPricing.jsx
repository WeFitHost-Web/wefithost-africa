...
const DomainCard = ({ title, domains, delay, currency }) => (
  ...
  >
    <div className="px-8 py-6 border-b border-slate-100">
      <h3 className="text-slate-900 font-black text-xl">{title}</h3>
    </div>
    <div>
      {domains.map((d, i) => (
        <div
          key={i}
          className="flex items-center justify-between px-8 py-5 border-b border-slate-100 last:border-b-0"
        >
          <div className="flex items-center gap-3">
            {d.badge && (
              <span className="bg-indigo-50 text-indigo-600 text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full">
                {d.badge}
              </span>
            )}
            <span className="text-slate-900 font-semibold">{d.tld}</span>
          </div>
